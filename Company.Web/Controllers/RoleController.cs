...
namespace Company.Web.Controllers
{
    ...
    public class RoleController : Controller
    {
              ...
              public async Task<IActionResult> Details(string id, string viewName = "Details")
        {
            ...
            if (viewName == "Update")
            {
                var UserViewModel = new RoleUpdateViewModelcs
                {
                    Id = role.Id,
                    Name = role.Name
                };
        ...
        [HttpPost]
        public async IActionResult Update(string id, RoleUpdateViewModelcs roleModel)
        {
            if (id != roleModel.Id)
            {
                return NotFound();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    if (result.Succeeded)
                    {
                        _logger.LogInformation("user updated succeesfully");
                        return RedirectToAction("Index");
                    }
                    foreach (var item in result.Errors)
                    {
